...
if (isset($_GET['no'])) {
    ...
    if ($result) {
        ...
        if ($delete_result) {
            ...
            header("Location: cuti_waitapp.php?success=1&message=Cuti dengan nomor $no_cuti telah dihapus.");
            exit();
        } else {
            // Jika terjadi kesalahan saat menghapus, kembalikan dengan pesan error
            header("Location: cuti_waitapp.php?error=1&message=Gagal menghapus cuti. Silakan coba lagi.");
            exit();
        }
    } else {
        // Jika data cuti tidak ditemukan, kembalikan dengan pesan error
        header("Location: cuti_waitapp.php?error=2&message=Cuti dengan nomor $no_cuti tidak ditemukan.");
        exit();
    }
} else {
    // Jika parameter nomor cuti tidak ditemukan, kembalikan dengan pesan error
    header("Location: cuti_waitapp.php?error=3&message=Nomor cuti tidak valid.");
    exit();
}
?>
