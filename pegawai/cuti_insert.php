...
if ($durasi > $jml) {
	...
	// Jika semuanya OK atau tidak ada file yang diunggah, lakukan unggah file (jika ada) dan simpan informasi ke database
	if ($uploadOk == 1 || !isset($_FILES["file_pendukung"]["name"]) || $_FILES["file_pendukung"]["name"] == "") {
		// Jika ada file yang diunggah, lakukan pengunggahan file
		if (isset($_FILES["file_pendukung"]["name"]) && $_FILES["file_pendukung"]["name"] != "" && move_uploaded_file($_FILES["file_pendukung"]["tmp_name"], $targetFile)) {
			// File berhasil diunggah, simpan informasi ke database dengan file
			$sql = "INSERT INTO cuti (no_cuti, npp, tgl_pengajuan, tgl_awal, tgl_akhir, durasi, keterangan, manager, stt_cuti, file_pendukung) 
                VALUES ('$id','$npp','$ajuan','$mulai','$akhir','$durasi','$ket','$mng','$stt','$fileName')";
		} else {
			// Tidak ada file yang diunggah atau gagal mengunggah file, simpan informasi ke database tanpa file
